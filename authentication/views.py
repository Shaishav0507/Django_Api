    ...
    def post(self, request):
        ...
        if not User.objects.filter(username=username).exists():
            if not User.objects.filter(email=email).exists():
                ...
                link = reverse('activate',kwargs={'uid':uid, 'token':account_activation_token.make_token(user)})
...
class RequestPasswordResetEmail(View):
    ...
    def post(self, request):
        ...
        if user.exists():
            ...
            email = EmailMessage(
                email_subject,
                'Hi there, please clink the link given below to reset your password \n' + reset_url,
                'noreply@gamil.com',
                [email],
            )
            EmailThread(email).start()

        messages.success(request, 'We have sent you the email to reset password')
